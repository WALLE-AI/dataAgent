...
def async_reranker():
    ...
    thread = threading.Thread(target=run_async, args=(RankerApi.async_reranker_documents(query,documents),reranker_docs))
    thread.start()
    thread.join()  # 等待线程完成
    # embedding_response = EmbeddingApi.asyc_embed_query(test) #'Vector_index_e570e853-291a-4338-9e83-d9762c22a1bb_Node'
    loguru.logger.info(f"reranker:{reranker_docs}")
...
if __name__ == "__main__":
    ...
    doc_res = ["你是谁","你能够做什么","建筑大模型解决质量安全隐患问题","什么是建筑大模型，该模型具备该领域什么样特性"]
    # async_reranker()
    # test_result = EmbeddingApi.asyc_embed_documents(doc_res)
    # loguru.logger.info(f"embedding:{test_result}")
    # documents = []
    # for doc in doc_res:
    #     metadata = {"doc_id":str(uuid.uuid4())}
    #     documents.append(Document(page_content=doc,metadata=metadata))
    # reranke_top = RankerApi.async_reranker_documents(test,documents)
    # loguru.logger.info(f"reranker:{reranke_top}")
